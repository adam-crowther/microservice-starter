...
import com.acroteq.ticketing.customer.service.domain.exception.CustomerSaveFailedException;
import com.acroteq.ticketing.customer.service.domain.exception.CustomerUpdateFailedException;
...
class CustomerCommandProcessor {
  ...
  CustomerEvent createCustomer(final CreateCustomerCommandDto createCustomerCommandDto) {
    ...
    if (savedCustomer == null) {
      throw new CustomerSaveFailedException();
    }
  ...
  CustomerEvent updateCustomer(final UpdateCustomerCommandDto updateCustomerCommandDto) {
    ...
    final Customer savedCustomer = customerRepository.save(customer);
    if (savedCustomer == null) {
      throw new CustomerUpdateFailedException(customer.getId());
    }
