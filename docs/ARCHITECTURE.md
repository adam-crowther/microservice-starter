...
> We do **not** allow microservices to cascade a request to more requests to other microservices. Cascading requests
> generate exponential complexity in the asynchronous message API design and error handling, and when things go wrong
> they make troubleshooting extremely difficult.<br>
> To avoid this we use the Orchestrator Pattern: the 'Orchestrator' microservice controls the Saga Flow and sends
> requests, and the other microservices execute their business logic according to the requests and send responses back
> to the Orchestrator.  

- See also [Messaging - Design Pattern: Saga](MESSAGING.MD#design-pattern-saga)

## CQRS - Command Query Responsibility Segregation

- For each Entity, we define a 'Primary' (="**Command**") microservice that is allowed to change that entity.
- We are strict that there can only be one 'Primary' microservice for each Entity.
- All other microservices are 'Replicas' of that entity, and can only perform read operations (="**Query**").
...
- Data entities are 'replicated' across domain boundaries (ie from the data repository of a 'Primary'
  microservice to that of the other microservices) through _Entity Event Messages_.
- Each microservice therefore has a data entity model which is divided into two parts: 'Primary Data' and 'Replicated
  Data'.
...
- Exception handling: unexpected exceptions in the Entity Domain Event listeners are handled by the standard Spring
  retry mechanism, with exponential backoff. If all retries fail, then the message is forwarded to a dead
...
- Entity Domain Events follow the `Kafka Streams KTable`/`KSQL query` conventions:
    ...
    - The message value contains the Entity's full schema, including all attributes, id and version, and including the
      audit createdTimestamp and lastModified Timestamp from the 'Primary' database. For security reasons, it does NOT
...
- See also [Messaging - Design Pattern: CQRS](MESSAGING.MD#design-pattern-cqrs)
