package com.acroteq.food.ordering.system.kafka.producer.service.impl;

import com.acroteq.food.ordering.system.domain.valueobject.BaseId;
import com.acroteq.food.ordering.system.kafka.producer.exception.KafkaProducerException;
...
@Component
public class KafkaProducerImpl<I extends BaseId<?>, V extends SpecificRecordBase>
    implements KafkaProducer<I, V> {
  ...
  private void handleSendResult(final SendResult<String, V> sendResult, final Throwable exception) {
    if (exception == null) {
      ...
      log.info("Received new metadata. " + EOL +
               "Topic:" + recordMetadata.topic() + EOL +
               "Partition: " + recordMetadata.partition() + EOL +
               "Offset: " + recordMetadata.offset() + EOL +
