...
class ValidationResultSpec extends Specification {
  ...
  def 'builder with failures should return a rejected result'() {
    ...
    then:
    result.failures == [FAILURE_MESSAGE_1, FAILURE_MESSAGE_2, FAILURE_MESSAGE_3]
    result.approvalStatus == REJECTED
    !result.pass
    result.logOutput == FAILURE_MESSAGE_1 + ',' + FAILURE_MESSAGE_2 + ',' + FAILURE_MESSAGE_3
  }
  ...
  def 'builder already containing the given message should not add a second failure message'() {
    ...
    then:
    result.failures == [FAILURE_MESSAGE_1]
    result.approvalStatus == REJECTED
    !result.pass
    result.logOutput == FAILURE_MESSAGE_1
  }
  ...
  def 'build failure with parameter should be formatted correctly'() {
    ...
    then:
    result.failures == [FAILURE_MESSAGE_WITH_PARAMETER]
    result.approvalStatus == REJECTED
    !result.pass
    result.logOutput == FAILURE_MESSAGE_WITH_PARAMETER
  }
  ...
  def 'static constructor method fail() with a null message and parameter should throw a NullPointerException'() {
    when:
    ValidationResult.fail(null, PARAMETER)
  ...
  def 'combine results with 2 failed results should be rejected'() {
    ...
    then:
    result.failures == [FAILURE_MESSAGE_1, FAILURE_MESSAGE_2]
    result.approvalStatus == REJECTED
    !result.pass
    result.logOutput == FAILURE_MESSAGE_1 + ',' + FAILURE_MESSAGE_2
  }
  ...
  def 'combine results with a null parameter should throw a NullPointerException'() {
    when:
    ValidationResult.combine(result1, result2)
